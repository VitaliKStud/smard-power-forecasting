...
tranfomed_power_consum <- cleaned_power_consum |>
  mutate(PowerConsum = PowerConsum)

train_power_consum <- tranfomed_power_consum |>
  filter(year(DateIndex) > 2020 & (year(DateIndex) < 2024))

test_power_consum <- tranfomed_power_consum |>
  select(
    # WorkDay,
    WorkDay,
...
fit <- train_power_consum |>
  model(
    ...
    ARIMA_A=ARIMA(PowerConsum, stepwise = FALSE, approx = FALSE, greedy=FALSE),
