...
working_colors <- c("1" = palette()[1], "0" = palette()[2])
...
fit <- train_power_consum |>
  model(
    TSLM = TSLM(PowerConsum ~
          ...
          + LastDayWasHolodiayAndNotWeekend
          + NextDayIsHolidayAndNotWeekend
          + fourier(period = "day", K = 12) # 8 original
          + fourier(period = "week", K = 24) # 5 original
...
all_forecasts_list <- fc$all_forecasts
...
ggplot(data = filtered_forecast, aes(x=DateIndex, y=.mean, color=.model)) +
  geom_line()
...
filtered_best_forecast <- all_forecasts |>
  filter(.model == best_forecast$.model) 
#   left_join(cleaned_power_consum |> rename(RealPowerConsum = PowerConsum,
#                                            WorkingDay = WorkDay), by = "DateIndex")
# 
# ggplot()+
#   geom_point(data = filtered_best_forecast, aes(x=RealPowerConsum, y=.mean, color=WorkDay)) +
#   geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red", lwd=1)


filtered_best_forecast |>
  autoplot()
