...
test_power_consum <- cleaned_power_consum |>
  filter((year(DateIndex) == 2024) & (month(DateIndex) == 1)) |>
  select(
    # WorkDay,
    Holiday,
...
fc <- fit |>
  forecast(new_data = test_power_consum) |>
  mutate(.mean = .mean,
...
rmse <- sqrt(mean((results$RealPowerConsum - results$Forecast)^2, na.rm = TRUE))
mape <- mean(abs((results$RealPowerConsum - results$Forecast) / results$Actual), na.rm = TRUE) * 100
mae <- mean(abs(results$RealPowerConsum - results$Forecast), na.rm = TRUE)
